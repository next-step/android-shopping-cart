...
- 아이템의 + 버튼 클릭하면 리컴포지션이 발생하도록 하기 위해서 StateFlow를 사용하려고 시도하다가 각 아이템별 버튼에 대한 클릭 관리가 어려워서 ProductActivity에서
      mutableStateOf를 사용하고 있습니다. 그런데 리컴포지션만을 위해서 해당 상태를 ProductsList까지 전달하는 것이 좋은 구조가 아닌 것 같습니다. 실제로
      ProductsList에서 buttonClickState를 사용하지는 않기 때문입니다. 리컴포지션만을 위한 이런 방식도 사용해도 괜찮을지, 더 좋은 방법은 어떤 것이 있는지 궁금합니다.
- 
