...
### 🚀 4단계 - 장바구니(수량)
- [x] 버그 수정 : 장바구니에 아이템이 담기지 않는 문제 수정하기
- [x] 이전 피드백 반영 : CartItem의 totalPrice get으로 매번 값을 계산하지 않고, state로 저장하도록 수정
- [x 상품 목록에서 장바구니에 담을/담긴 상품의 수량을 조절할 수 있다
  - [x] 장바구니에 담기지 않은 아이템에 대해 장바구니에 담을 수 있는 + 버튼을 추가한다.
  - [x] 장바구니에 담긴 아이템은 수량 조절 UI를 보여준다.
  - [ ] (optional) + 버튼에서 수량 조절 UI로 변경할 때 B마트처럼 애니메이션을 이용한다.
- [x] 상품 목록 화면에서 장바구니에 담긴 수량 만큼 우측 상단에 수량을 뱃지로 표시한다.

### 🚀 5단계 - +@
- [x] TabLayout과 ViewPager를 이용해 상품 목록 화면에 카테고리 기능 추가
- [ ] 테스트 코드 작성하기
